ALTER TABLE public.shuttle_signups
  ADD COLUMN IF NOT EXISTS departure_plan text,
  ADD COLUMN IF NOT EXISTS florence_rsvp text;

DROP FUNCTION IF EXISTS public.book_shuttle(text,integer,text,text,boolean,text,text,text[]);
...
GRANT EXECUTE ON FUNCTION public.book_shuttle(text,integer,text,text,boolean,text,text,text[],text,text) TO anon, authenticated;
