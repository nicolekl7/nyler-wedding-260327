-- Make shuttle wave capacity editable from the admin page instead of hardcoded.

CREATE TABLE public.shuttle_capacity (
  direction text NOT NULL CHECK (direction IN ('arrival','departure')),
  wave text NOT NULL CHECK (wave IN ('wave_1','wave_2')),
  capacity int NOT NULL CHECK (capacity >= 0),
  PRIMARY KEY (direction, wave)
);

INSERT INTO public.shuttle_capacity (direction, wave, capacity) VALUES
  ('arrival', 'wave_1', 26),
  ('arrival', 'wave_2', 26),
  ('departure', 'wave_1', 22),
  ('departure', 'wave_2', 22);

GRANT SELECT, UPDATE ON public.shuttle_capacity TO anon, authenticated;
GRANT ALL ON public.shuttle_capacity TO service_role;

ALTER TABLE public.shuttle_capacity ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Anyone can view shuttle capacity"
  ON public.shuttle_capacity FOR SELECT USING (true);
CREATE POLICY "Anyone can update shuttle capacity"
  ON public.shuttle_capacity FOR UPDATE USING (true) WITH CHECK (true);
...
BEGIN
  ...
  IF _arrival_wave <> 'none' THEN
    SELECT capacity INTO cap FROM public.shuttle_capacity WHERE direction = 'arrival' AND wave = _arrival_wave;
  ...
  IF _departure_wave <> 'none' THEN
    SELECT capacity INTO cap FROM public.shuttle_capacity WHERE direction = 'departure' AND wave = _departure_wave;
